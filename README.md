...
faster and more energy efficient. Due to it training structure, each pre-trained model can be viewed as a module, making it easier to
achieve latent space transfer between domains. This implementation is based from this [paper](https://arxiv.org/pdf/1902.08261.pdf)
but only on one domain music (For experimentation purposes). 

For better at capturing coherence music melody, this repo also includes the implementation of [MidiMe model](https://magenta.tensorflow.org/midi-me)
from Google open source [Magenta](https://github.com/tensorflow/magenta) but implemented in Python instead of JavaScript from original model. 
For more detail behind the model, please take a look at this [paper](https://research.google/pubs/pub48628/).
...
but we also need to load MusicVAE model instead of only loading LCMusicVAE model.
...
This is just a [MidiMe model](https://magenta.tensorflow.org/midi-me) but implemented in Python.

#### Training
Similar to `LCMusicVAE`, we just need to change the `config` value to `ae-cat-mel_2bar_big`:
```sh
python midime_train.py \
...
#### Sample
Same thing applies here, we just need to change `config` parameter to `ae-cat-mel_2bar_big`:
